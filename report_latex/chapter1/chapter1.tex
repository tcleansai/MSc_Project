...
\item end
\end{enumerate}
Following is the warping result, the image was not highly distorted as the face is basically facing to the frontal. However, it is easy to notice the face is a little rotated to the right and there is a small change on the shape of mouth.
\begin{figure}[ht]
\centering
...
As it is known large margin classifiers are sensitive to the way features are scaled, it's better to normalize either the data or the kernel function \cite{ben2010user}. Feature of an image is represented by a vector, the number in the vector would influence the weight of feature in this dimension. As I would like to treat each dimension similar, I scale the number in the range of $[0,1]$.
