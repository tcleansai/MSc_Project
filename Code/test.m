...
for i = 1:10
    ...
    [features_train,videoSeq_test] = test_1(...
    ...
    dataX = dataX-repmat(mean_train,minSize*3,1);
    std_train = std(dataX);
    dataX = dataX./(repmat(std_train,minSize*3,1)+eps);
    min_train = min(dataX);
    dataX = dataX-repmat(min_train,minSize*3,1);
    max_train = max(dataX);
    dataX = dataX./repmat(max_train,minSize*3,1);
    % mix traning data
    n = minSize*3;
    tmp = randperm(n);
    dataX = dataX(tmp,:);
...
tmpPath = '/vol/bitbucket/aaaaaaaaaaaaaaaaaa/data/result/';

save(strcat(tmpPath,'result1',num2str(nBlock),fea_param,mat2str(z(param_idx,:)),'.mat'),...
